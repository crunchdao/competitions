...
def check(
    ...
    with tracer.log("Ensure contains package"):
        ...
        crunch_synth = find_requirement_by_name(python_requirements, "crunch-numinous")
        if crunch_synth is None:
            raise ParticipantVisibleError("`requirements.txt` file must include the `crunch-numinous` package.")
